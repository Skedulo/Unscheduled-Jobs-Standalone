...
export default function wrapper(
  ...
  formSaveFunc,
  widgets,
  onLoadCallback = () => console.info("--> View Ready <--")
) {
  ...
  return function (formData, deviceCache) {
    ...
    if (location.protocol.indexOf("http") > -1) {
      ...
      widgets.GraphQL = ({ query, variables }) =>
        graphiQl.executeQuery(query, variables).then((res) => res.data);
    }

    // format query data
    widgets.originGrapthQl = widgets.GraphQL;
    widgets.GraphQL = (parameters) =>
      widgets.originGrapthQl(parameters).then(graphiQl.formatResponseData);

    // setup live data service
    const liveDataService = main.authData
      ? new LiveDataService(main.authData)
      : null;
    ...
    ReactDOM.render(
      <Provider store={store}>
        <Context.Provider
          value={{ main, common, widgets, deviceCache, liveDataService }}
        >
          <MainComponent />
        </Context.Provider>
      </Provider>,
